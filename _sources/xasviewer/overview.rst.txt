...
Viewer are:
  ...
  1. XAS Viewer organizes all XAS Spectra into *Groups* of data.  In fact, the
  ...
  2. To the extent possible, all data processing and analysis steps, and even
     most of the visualization steps in XAS Viewer will be done in the Larch
     ...
     want to know exactly what XAS Viewer is "really doing", you can open the
     Larch Buffer and see the commands being executed.


With those overall goals in mind, the main features of XAS Viewer can be broken
...
since some Groups of data will be generated by XAS Viewer, the list here really
...
.. figure:: ../_images/XASViewer_Main.png
    ...
    XASViewer showing the File/Group list on the left-hand side and the set of
...
XAS Viewer opens with a main window as shown in :numref:`fig_xasviewer_top`,
...
While the List of Spectra is shown on the left-hand side of the XAS Viewer
window, the right-hand side shown in :numref:`fig_xasviewer_top` contains
...
normalization (for more detail, see :ref:`xasviewer_preedge`).  Other notebook
tabs include panels for fitting pre-edge peaks (:ref:`xasviewer_peakfit`),
Linear Combination Analysis (:ref:`xasviewer_lincombo`), Principal Component
Analysis (:ref:`xasviewer_pca`), Advanced Linear Regression
(:ref:`xasviewer_regression`), EXAFS Data Processing
(:ref:`xasviewer_exafs_bkg` and :ref:`xasviewer_exafs_fft`), and Feff fitting
(:ref:`xasviewer_feffit`).  These panels generally assume that data has been
...
used in many of the work panels of XAS Viewer, including the Normalization
panel.
...
Interactive data visualization is an important component of XAS Viewer.  On
startup, a plot window will be shown for plotting X/Y line plots, the most
common way to plot XAS data.  XAS Viewer will display many different datasets
...
more.  While XAS Viewer starts up with one plot window, some analysis tasks
...
XAS Viewer starts up with one plot window.  Some analysis tasks will bring up a
...
XAS Viewer can import XAS data in the following formats
   ...
   * plain text data data files with data arrays in column format
     (:ref:`xasviewer_plaintext`), including data files from most XAS beamlines
     at most facilities.
...
For saving and exporting data, XAS Viewer can save multiple XAS spectra as:
...
For XANES analysis, XAS Viewer supports the following data analysis processes:
...
Each of these is given its own "Tab" in the main XAS Viewer window.

===================================
EXAFS Processing
===================================

For EXAFS analysis, the EXAF XANES analysis, XAS Viewer supports the following data analysis processes:
