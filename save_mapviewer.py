...
class TriColorMapPanel(wx.Panel):
    ...
    def onShow3ColorMap(self, event=None, new=True):
        ...
        rscale = 255.0/self.rscale.GetValue()
        gscale = 255.0/self.gscale.GetValue()
        bscale = 255.0/self.bscale.GetValue()
        if self.rauto.IsChecked():  rscale = 255.0/rmap.max()
        if self.gauto.IsChecked():  gscale = 255.0/gmap.max()
        if self.bauto.IsChecked():  bscale = 255.0/bmap.max()
        print '3 color map ', rscale, gscale, bscale
