#!/usr/bin/env python
"""
read_feffdat provides the following function to larch:
    ...
    def read(self, filename):
        ...
        for line in lines:
            ...
            if line.startswith('k') and line.endswith('real[p]@#'):
                ...
                continue
            #
            if mode == 'header' and line.startswith('Abs') or line.startswith('Pot'):
...
class FeffPathGroup(larch.Group):
    ...
    def _calcchi(self, kmax=20, kstep=0.05, degen=None, s02=None, 
        ...
        # e0-shifted k:
        en = k*k - e0*ETOK
        shiftedk = np.sign(en)*np.sqrt(abs(en))

        pha = np.interp(shiftedk, self._dat.k, self._dat.pha)
        amp = np.interp(shiftedk, self._dat.k, self._dat.amp)
        rep = np.interp(shiftedk, self._dat.k, self._dat.rep)
        lam = np.interp(shiftedk, self._dat.k, self._dat.lam)

        if debug:
            self.debug_k   = shiftedk
            self.debug_pha = pha
            self.debug_amp = amp
            self.debug_rep = rep
            self.debug_lam = lam
