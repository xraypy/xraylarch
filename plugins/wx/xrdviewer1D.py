...
class XRD1DViewerFrame(wx.Frame):
    ...
    def __init__(self, conffile=None,  use_scandb=False, _larch=None, **kwds):
        ...
        self.SetTitle('GSE 1D XRD Viewer')
        self.SetFont(Font(9))
    ...
    def createMenus(self):
        ...
        hmenu = wx.Menu()
        ID_ABOUT = wx.NewId()
        hmenu.Append(ID_ABOUT, '&About', 'About GSECARS XRD Viewer')
...
#                print('This file already has XRD data. None will be added.')
    ...
    def onTimer(self, event):
        ...
        self.message('Viewer Timer Processing %s:  row %i of %i' % (fname, irow, nrow))
        if self.h5convert_done:
            self.htimer.Stop()
            self.h5convert_thread.join()
            if fname in self.files_in_progress:
                self.files_in_progress.remove(fname)
            self.message('ViewerTimer Processing %s: complete!' % fname)
...
class XRD1DViewer(wx.App):
    ...
    def createApp(self):
        frame = XRD1DViewerFrame(use_scandb=self.use_scandb)
        frame.Show()
...
class DebugViewer(XRD1DViewer, wx.lib.mixins.inspection.InspectionMixin):
    def __init__(self, **kws):
        XRD1DViewer.__init__(self, **kws)
