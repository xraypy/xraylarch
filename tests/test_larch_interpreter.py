...
def test_larch_interpreter():
    session = LarchSession()
    do = session.run
    do("a_dict = {'a': 1, 'b': 2, 'c': 3, 'd': 4}")
    do("a_list = ['a', 'b', 'c', 'd', 'o']")
    do("a_tuple = (5, 'a', 'x')")
    do("a_string = 'hello world'")
    do("a_ndarray = 5*arange(20)")
    do("b_ndarray = arange(200).reshape(10, 20)")
    do("c_ndarray = arange(20).reshape(4, 5)")
    results = {"a_dict['a']": 1,
               "a_dict['d']": 4,
               "a_list[0]": 'a',
               "a_list[1]": 'b',
               ...
               "a_ndarray[2]": 10,
               "a_ndarray[4]": 20,
               "b_ndarray[1:3,5:7]": np.array([[25,26], [45,46]]),
               "c_ndarray[:,3]": np.array([3, 8, 13, 18]),
               "c_ndarray[...,1]": np.array([1, 6, 11, 16]),
               "c_ndarray[1,:]": np.array([5, 6, 7, 8, 9])
                   }
...
""")
    ...
    assert session.symtable.get_symbol('n') == 8
...
#endwhile
""")
    assert session.symtable.get_symbol('n') == 4
...
#endwhile
""")
    assert session.symtable.get_symbol('n') == 7
#
#         self.trytext("""
# n=0
# while n < 10:
