...
class AthenaGroup(Group):
    ...
    def edit_groups(self):
        ...
        self._labs = [lab for lab in self.groups.keys()]
        self._sels = [grp.sel for grp in self.groups.values()]

        # store widgets
        self._widgets = [AthenaGroupWidget(idx, lab, grp.sel) for idx, (lab, grp) in enumerate(self.groups.items())]

        #make layout and display
        iLabs = [wdg.wlab for wdg in self._widgets]
        iSels = [wdg.wsel for wdg in self._widgets]
