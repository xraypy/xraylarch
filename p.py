...
class ParameterPanel(wx.Panel):
    ...
    """
    def __init__(self, parent, param, size=(80, -1), show_name=True, precision=4, **kws):
        self.param = param
        ...
        if show_name:
            self.wids.name = wx.StaticText(self, label="%s: " %  param.name, size=(len(param.name)*8, -1))
...
class TestFrame(wx.Frame):
    def __init__(self, parent=None, size=(-1, -1)):
        ...
        param2 = Parameter(value=1.23, vary=True, min=0.5, max=2.0, name='peak1_center')
        param3 = Parameter(value=1.23, vary=True, min=0.5, max=2.0, name='peak1_sigma')

        panel.Add(ParameterPanel(panel, param1),  style=LEFT)
        # panel.NewRow()
        panel.Add(ParameterPanel(panel, param2),   style=LEFT)
        panel.Add(ParameterPanel(panel, param3),   style=LEFT)
        panel.pack()
        self.createMenus()
